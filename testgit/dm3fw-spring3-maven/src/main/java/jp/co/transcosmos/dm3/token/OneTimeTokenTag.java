 ...
 * </pre>
*/
public class OneTimeTokenTag implements Tag {
    private static final Log log = LogFactory.getLog(OneTimeTokenTag.class);
     ...
     * 存在しない場合は新規に ID を採番してセッション、リクエストスコープに格納する。<br/>
    ...
    public int doStartTag() throws JspException {
    	...
    	// ページスコープ内に、既にトークンが生成されているかをチェックする。
    	String tokenId = (String)pageContext.getAttribute(this.tokenName);
    	...
    	if (tokenId == null || tokenId.length() == 0){
    		...
    		// 新たなトークンを生成した場合、ページスコープ、セッションに格納する。
    		pageContext.setAttribute(this.tokenName, tokenId);
    		pageContext.getSession().setAttribute(this.tokenName, tokenId);
    		
    		log.info("Generate One Time Token : name=" + this.tokenName + "," +
		...
		try {
            ...
            throw new JspTagException("Error writing One Time Toekn");
        }
        return SKIP_BODY;
    }
