...
<script type="text/javascript" src="<c:out value='${commonParameters.resourceRootUrl}'/>common/js/efoBase.js"></script>
<script type="text/javascript" src="<c:out value='${commonParameters.resourceRootUrl}'/>common/js/efoMember.js"></script>
			...
			<div class="itemBlockA01 spMb00">
				...
				<div class="columnInner">
					<p class="mb10"><span class="mustIcon">必須</span> 項目は必ずご入力ください。</p>
						<table class="tableBlockA01">
							...
							<tr>
									<th><label for="newPassword01">パスワード<span class="mustIcon">必須</span></label></th>
									<td class="pb00">
										<c:import url="/WEB-INF/jsp/front/include/panaValidationFieldErrors.jsh" >
							...
							<tr>
								<th></th>
								<td><label for="togglePassword"><input id="togglePassword" type="checkbox">パスワード表示</label></td>
							</tr>
						</table>
					</div><!-- /.columnInner -->
				</div><!-- /.itemBlockA01 -->
						...
						<table class="tableBlockA01">
							<tr>
								...
								<td>
									...
									<div>姓 <input type="text" id="memberLname" name="memberLname" value="<c:out value="${inputForm.memberLname}"/>" placeholder="例：松下" class="inputType05 <c:if test="${lnameHasError}">error</c:if>" maxlength="<dm3lookup:lookup lookupName="inputLength" lookupKey="mypage.input.memberLname" defaultValue="30"/>">
									名 <input type="text" id="memberFname" name="memberFname" value="<c:out value="${inputForm.memberFname}"/>" placeholder="例：太郎" maxlength="<dm3lookup:lookup lookupName="inputLength" lookupKey="mypage.input.memberFname" defaultValue="30"/>" class="inputType05 <c:if test="${fnameHasError}">error</c:if>" ></div>
								</td>
							</tr>
							<tr>
								...
								<td>
									...
									<div>姓 <input type="text" id="memberLnameKana" name="memberLnameKana" value="<c:out value="${inputForm.memberLnameKana}"/>" placeholder="例：マツシタ"  maxlength="<dm3lookup:lookup lookupName="inputLength" lookupKey="mypage.input.memberLname" defaultValue="30"/>" class="inputType05 <c:if test="${lnameKanaHasError}">error</c:if>" >
									名 <input type="text" id="memberFnameKana" name="memberFnameKana" value="<c:out value="${inputForm.memberFnameKana}"/>" placeholder="例：タロウ" maxlength="<dm3lookup:lookup lookupName="inputLength" lookupKey="mypage.input.memberFname" defaultValue="30"/>" class="inputType05 <c:if test="${fnameKanaHasError}">error</c:if>" ></div>
								</td>
							</tr>
							...
							<table class="tableBlockA01">
								<tr>
									...
									<td>
										...
										<div>
											<select name="hopePrefCd" id="hopePrefCd">
												<option value="">選択してください</option>
												<c:forEach items="${prefMstList}" var="prefMst">
				...
				<div class="itemBlockA03">
					...
					<div class="columnInner">
						...
						<p class="center mb15"><button type="submit" onClick="linkToUrl('<c:out value="${pageContext.request.contextPath}"/>/account/member/new/confirm/', '');" name="confirmBtn" id="confirmBtn" class="primaryBtn01" >入力内容を確認する</button></p>
